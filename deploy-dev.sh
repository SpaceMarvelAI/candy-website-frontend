...
python3 scripts/build_scorecard.py dev "$DEPLOY_TAG" "$GIT_SHA" "$DEPLOYER" "$ALARMS_RESULT" --version="$NEXT_VERSION" \
  || echo "  (scorecard build failed — non-fatal, the deploy above already succeeded)"
echo "Report saved to S3: s3://smai-deploy-scorecards/candy-website-frontend-dev/$(date -u +%Y)/$DEPLOY_TAG.json (version $NEXT_VERSION)"
